...
/********* From the file "DBSQL_ASSERT.h" ************************************/
void myDBSQL_ASSERT();
#ifndef NDEBUG
#  define DBSQL_ASSERT(X) if(!(X))myDBSQL_ASSERT(__FILE__,__LINE__)
#else
#  define DBSQL_ASSERT(X)
#endif
...
struct action *ap2;
{
  ...
  if( rc==0 ){
    DBSQL_ASSERT( ap1->type==REDUCE || ap1->type==RD_RESOLVED || ap1->type==CONFLICT);
    DBSQL_ASSERT( ap2->type==REDUCE || ap2->type==RD_RESOLVED || ap2->type==CONFLICT);
...
/********************** From the file "DBSQL_ASSERT.c" ****************************/
/*
** A more efficient way of handling DBSQL_ASSERTions.
*/
void myDBSQL_ASSERT(file,line)
char *file;
int line;
{
...
struct symbol *errsym;   /* The error symbol (if defined.  NULL otherwise) */
{
  ...
  int errcnt = 0;
  DBSQL_ASSERT( apx->sp==apy->sp );  /* Otherwise there would be no conflict */
  if( apx->type==SHIFT && apy->type==REDUCE ){
    ...
    if( spy==0 || spx->prec<0 || spy->prec<0 ){
      ...
      DBSQL_ASSERT( spx->prec==spy->prec && spx->assoc==NONE );
      apy->type = CONFLICT;
      errcnt++;
    }
...
struct config *cfp;
{
  ...
  for(; cfp; cfp=nextcfp){
    nextcfp = cfp->next;
    DBSQL_ASSERT( cfp->fplp==0 );
    DBSQL_ASSERT( cfp->bplp==0 );

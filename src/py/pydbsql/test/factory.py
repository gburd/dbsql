...
# Tests for the various factories in pydbsql

import unittest
import pydbsql.dbapi2 as dbsql

class MyConnection(dbsql.Connection):
    def __init__(self, *args, **kwargs):
        dbsql.Connection.__init__(self, *args, **kwargs)
...
class ConnectionFactoryTests(unittest.TestCase):
    def setUp(self):
        self.con = dbsql.connect(":memory:", factory=MyConnection)
...
class RowFactoryTests(unittest.TestCase):
    ...
    def CheckDbsqlRowIndex(self):
        self.con.row_factory = dbsql.Row
        row = self.con.execute("select 1 as a, 2 as b").fetchone()
        self.failUnless(isinstance(row,
                                   dbsql.Row),
                        "row is not instance of dbsql.Row")
    ...
    def CheckDbsqlRowAsTuple(self):
        """Checks if the row object can be converted to a tuple"""
        self.con.row_factory = dbsql.Row
        row = self.con.execute("select 1 as a, 2 as b").fetchone()
        t = tuple(row)

    def CheckDbsqlRowAsDict(self):
...
class TextFactoryTests(unittest.TestCase):
    ...
    def CheckOptimizedUnicode(self):
        self.con.text_factory = dbsql.OptimizedUnicode
        austria = unicode("Österreich", "latin1")
